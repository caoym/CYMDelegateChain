 ...
 */
@interface CYMDelegateChain : NSObject
 ...
 */
#define CYMDelegateChainInsert(root_,delegate_, owner_)\
root_ = (__typeof(root_))[CYMDelegateChain insert:delegate_ before:root_ owner:owner_];

/**
 * 删除已存在的delegate
 */
#define CYMDelegateChainRemove(root_,delegate_, owner_)\
root_ = (__typeof(root_))[CYMDelegateChain remove:delegate_ from:root_ owner:owner_];
 ...
 * (利用TLS实现，只在当前线程有效)
 */
#define CYMDelegateChainContinue()\
[CYMDelegateChain continueChain:YES];

/**
 * 替换已存在的delegate
 */
#define CYMDelegateChainReplace(root_,src_,dest_, owner_)\
root_ = (__typeof(root_))[CYMDelegateChain replace:src_ with:dest_ from:root_ owner:owner_];
